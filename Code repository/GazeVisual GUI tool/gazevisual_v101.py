...
## and if you want to use it with this software. 
##
#######################################
...
#            plt.plot(tim_rel,gtt_ang)
#            plt.plot(tim_rel,get_ang)
        ...
        def zscore():
            ...
            #plt.scatter(tim_rel,yaw_gt)
            #plt.scatter(yaw_gt,y_data)
            self.T21.delete(1.0, END) 
        ...
        def yawpitch():
            ...
            #print max_yaw, min_yaw, max_pitch, min_pitch, mean_eyaw, mean_epit#, mean_eyaw,mean_epit
            f2 = Figure(figsize=(5.5,3.7), dpi=100)      
            f2.subplots_adjust(wspace = 0.4)
            self.canvas2 = FigureCanvasTkAgg(f2, page2)
            self.canvas2.show()
...
#                return
        ...
        def plothist_v1():
            ...
            lab1= "bin size: "+ bin1
            #np.histogram(all_er_45,bins)
            
            fs = Figure(figsize=(5.5,3.7), dpi=100)
            self.canvas = FigureCanvasTkAgg(fs, page3)
            self.canvas.show()
            fs.add_subplot(111).hist(diff_gz_v, bins, histtype='stepfilled', normed=False, color='r', alpha=0.4, label=lab1)
        ...
        def threedplot():
            ...
            p= ax.plot_surface(xnew, ynew , znew , rstride=40, cstride=60,cmap=cm.jet, antialiased=True)
...
#            
        ...
        def data_density():
            ...
            im= ax.hist2d(x_v, y_v, (bin2, bin2), cmap=plt.cm.jet)
            fs.colorbar(im[3])
            ...
            self.toolbar = NavigationToolbar2TkAgg( self.canvas, self.toolbar_frame )
            self.canvas._tkcanvas.grid(row=1,column=3,rowspan=5, columnspan=3, padx = 0,pady = 0, sticky=N)
            #gtt_angv1 #ground truth gaze angles
        ...
        def bubble():
            ...
            print len(mean_err)#[0:26]
            #inds =[1, 2, 3, 6,7, 10,11,12, 15, 16, 21, 22, 23, 26, 27]
            aoi_ind= [90*n for n in range(0,28)]
            #gaze angles at 28 aois
              #gaze angle at aoi points
            ...
            self.canvas = FigureCanvasTkAgg(fs, page3)
            self.canvas.show()
            fs.add_subplot(111).scatter(aoi_x1, aoi_y1, color='r', alpha=0.4)
            ...
            #fs.add_subplot(111).errorbar(aoi_x1, aoi_y1, yerr=meanvals, uplims=True)
        ...
        def box_plot():
           ...
           # f1.add_subplot(111).set_ylim([0,10 ])
            f1.add_subplot(111).xaxis.set_tick_params(labelsize=8)
            f1.add_subplot(111).yaxis.set_tick_params(labelsize=8)
...
#    
        ...
        #self.but_aoi=Button(page3, text='Plot',font='Lucida 10 bold', fg='black').grid(row=4,column=1,padx = 5,pady =0,sticky= NW)
...
#    #################### alter plot features ##########################
        ...
        self.but310=Button(page3, text='Refresh',font='Lucida 10 bold', height=1, width=5,fg='black').grid(row=7,column=5,padx =10,pady =0,sticky= NE)
...
user follows a ball appearing on screen """, font = 'Helvetica 10',justify="left", anchor= "w").grid(row=1,column=0, columnspan=3,padx = 0,pady =10, sticky = S)#, padx = 5, pady = 5,sticky= N )
        
        #self.f41 = LabelFrame(page4, text = 'Test UI', font = "Helvetica 12 bold", height = 400, width = 400).grid(row=0,column=0,rowspan=8, columnspan=3,padx = 10,pady = 5,sticky = NW)#.pack(fill=X,side=LEFT,padx=20,pady=5)
        self.f42 = LabelFrame(page4, text = 'LiveTracking', font = "Helvetica 12 bold", height = 200, width = 400).grid(row=8,column=0,rowspan=6, columnspan=3,padx = 10,pady = 5,sticky = NW)#.pack(fill=X,side=LEFT,padx=20,pady=5)
    ...
    Press Refresh to clear console area. """
        ...
        #self.texv46= Label(page4,  text= "Start with Test UI" ,font = 'Helvetica 12 bold',justify="left", anchor= "w").grid(row=8,column=0, padx = 15,pady =5, sticky = NW)
...
or with a Test UI to collect gaze data. """, font = 'Helvetica 10',justify="left", anchor= "w").grid(row=10,column=0, columnspan=3,padx = 0,pady =10, sticky = N)#, padx = 5, pady = 5,sticky= N )
        #self.texv42= Label(page4,  text= "Connect tracker" ,font = 'Helvetica 12 bold',justify="left", anchor= "w").grid(row=9,column=0, padx = 15,pady =5, sticky = SW)
        ...
        #self.texv43 = Label(page4,  text= "Start with Test UI" ,font = 'Helvetica 12 bold',justify="left", anchor= "w").grid(row=11,column=0, padx = 15,pady =5, sticky = NW)
        #self.texv44 = Label(page4,  text= "Show with UI-2" ,font = 'Helvetica 12 bold',justify="left", anchor= "w").grid(row=12,column=0, padx = 15,pady =5, sticky = NW)
        
        #self.but43=Button(page4, text='Connect',height=1,width=8,font='Lucida 10 bold', fg='black', command=file_save2).grid(row=9,column=1,padx = 10,pady =5,sticky= SW)
        #self.but44=Button(page4, text='Start tracking',width=20,font='Lucida 10 bold', fg='black',command= clear_text_field).grid(row=10,column=1, columnspan=2,padx = 10,pady =0,sticky= NW)
        ...
        #self.but44=Button(page4, text='Start',width=8,font='Lucida 10 bold', fg='black',command= clear_text_field).grid(row=12,column=1,padx = 10,pady =0,sticky= NW)
        
        #self.but43=Button(page4, text='Disconnect',height=1,width=8,font='Lucida 10 bold', fg='black', command=file_save2).grid(row=9,column=2,padx = 10,pady =5,sticky= SW)
        #self.but44=Button(page4, text='Stop',width=8,font='Lucida 10 bold', fg='black',command= clear_text_field).grid(row=10,column=2,padx = 10,pady =0,sticky= NW)
        #self.but43=Button(page4, text='Stop',height=1,width=8,font='Lucida 10 bold', fg='black', command=file_save2).grid(row=11,column=2,padx = 10,pady =0,sticky= NW)
        #self.but44=Button(page4, text='Stop',width=8,font='Lucida 10 bold', fg='black',command= clear_text_field).grid(row=12,column=2,padx = 10,pady =0,sticky= NW)
...
#        """ ,font = 'Helvetica 12 bold',justify="left", anchor= "w").grid(row=9,column=0, padx = 20,pady =25, sticky = NW)

GUI(t)
#t.iconbitmap('fella.ico')
